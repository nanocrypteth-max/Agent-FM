...
function generateTeamPlayers(opts: {
  ...
  for (const { position, count } of SQUAD) {
    for (let j = 0; j < count; j++) {
      ...
      switch (position) {
        case "GK":
          base = {
            pace: attr(quality - 10, v, rng),
            shooting: attr(20, 5, rng),
            passing: attr(quality - 5, v, rng),
            defending: attr(quality + 15, v, rng),
            stamina: attr(quality, v, rng),
          };
          break;
        case "DF":
          base = {
            pace: attr(quality, v, rng),
            shooting: attr(30, 8, rng),
            passing: attr(quality, v, rng),
            defending: attr(quality + 12, v, rng),
            stamina: attr(quality + 5, v, rng),
          };
          break;
        case "MF":
          base = {
            pace: attr(quality, v, rng),
            shooting: attr(quality, v, rng),
            passing: attr(quality + 12, v, rng),
            defending: attr(quality, v, rng),
            stamina: attr(quality + 10, v, rng),
          };
          break;
        case "FW":
          base = {
            pace: attr(quality + 12, v, rng),
            shooting: attr(quality + 12, v, rng),
            passing: attr(quality, v, rng),
  ...
  const avgOf = (pos: string, key: keyof GeneratedPlayer) => {
    ...
    return Math.round(
      pool.reduce((s, p) => s + (p[key] as number), 0) / pool.length,
    );
  };
