...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid tactics", details: parsed.error.flatten() }, { status: 400 });
  }
  ...
  if (fixture.status === "SIMULATED") {
    return NextResponse.json({ error: "Fixture already simulated, cannot change tactics" }, { status: 409 });
  }
  ...
  if (!userTeam) {
    return NextResponse.json({ error: "Neither team in this fixture is user-controlled" }, { status: 403 });
  }
  ...
  if (invalidIds.length > 0) {
    return NextResponse.json({ error: `Invalid player IDs: ${invalidIds.join(", ")}` }, { status: 400 });
  }

  const gkCount = userTeam.players.filter((p) => startingXI.includes(p.id) && p.position === "GK").length;
  if (gkCount !== 1) {
    return NextResponse.json({ error: `Squad must include exactly 1 GK, got ${gkCount}` }, { status: 400 });
  }

  if (new Set(startingXI).size !== 11) {
    return NextResponse.json({ error: "startingXI contains duplicate player IDs" }, { status: 400 });
  }
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!userTactics) {
    return NextResponse.json({ error: "No tactics submitted yet" }, { status: 404 });
  }
