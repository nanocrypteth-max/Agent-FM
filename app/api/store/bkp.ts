...
export async function POST(req: NextRequest) {
  ...
  if (!solanaWallet || !storePlayerId || !txHash) {
    return NextResponse.json(
      { error: "solanaWallet, storePlayerId, txHash required" },
      { status: 400 },
    );
  }
  ...
  const storePlayer = await prisma.storePlayer.findUnique({
    where: { id: storePlayerId },
  });
  ...
  try {
    ...
    if (received < Math.round(expectedSol * LAMPORTS * 0.99)) {
      return NextResponse.json(
        { error: `Expected ${expectedSol} SOL` },
        { status: 400 },
      );
    }
  } catch {
