...
export async function POST(
  ...
  // Both ready → notify clients to trigger simulate (host will call simulate)
  if (updated.hostReady && updated.guestReady) {
    await prisma.friendlyMatch.update({
      where: { id: lobby.id },
      data: { status: "PLAYING" },
    });
    await pusher.trigger(CHANNELS.friendly(params.code), EVENTS.BOTH_READY, {
      friendlyId: lobby.id,
    });
  }
  ...
  return NextResponse.json({
    ready: true,
    bothReady: updated.hostReady && updated.guestReady,
  });
}
