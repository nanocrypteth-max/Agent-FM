...
function TopupContent() {
  ...
  async function handleTopup(pack: (typeof TOPUP_PACKS)[0]) {
    ...
    try {
      ...
      if (res.ok) {
        setMsg({
          text: `✓ $${pack.usd.toLocaleString()} added to your balance!`,
          ok: true,
        });
        refetch();
      } else {
    ...
    <div className="page" style={{ maxWidth: 680, margin: "0 auto" }}>
      <div className="ws-hero">
        ...
        <p className="ws-subtitle">
          Purchase in-game USD to buy players in the transfer market. Rate:{" "}
          <strong style={{ color: "var(--ws-gold)" }}>1 SOL = $10,000</strong>
        </p>
      </div>
      ...
      >
        ℹ️ Transactions are processed on Solana devnet. SOL is sent to the game
        treasury. In-game USD is used exclusively within Agent FM for player
        transfers. It has no real-world monetary value.
      </div>
    </div>
  );
}
