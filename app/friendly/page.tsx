...
export default function FriendlyPage() {
  return <AuthWall><FriendlyContent /></AuthWall>;
}
  ...
  return (
    <div className="page">
      <div className="ws-hero" style={{ marginBottom: 20 }}>
        <div className="ws-badge"><span className="pulse-ball" />PvP</div>
        <h1 className="ws-title">Friendly Matches</h1>
        <p className="ws-subtitle">
          Challenge other managers to a friendly match. Create a lobby and share your 6-character code, or join an existing match.
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        ...
        <div className="panel" style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <p style={{ fontSize: 13, color: "var(--ink-dim)", lineHeight: 1.6 }}>
            Start a new lobby and share the 6-character code with a friend. Lobby expires in 30 minutes if no one joins.
          </p>
          <button
            onClick={createLobby}
            disabled={creating}
        ...
        <div className="panel" style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <input
            ...
            placeholder="ABCDEF"
            style={{
              padding: "12px", borderRadius: 6, border: "1px solid var(--border)",
              background: "var(--panel-bg)", color: "var(--ink)",
              fontFamily: "var(--mono)", fontSize: 20, letterSpacing: 4,
          ...
          <button
            ...
            style={{
              padding: "12px", borderRadius: 8, border: "none",
              background: joinCode.length === 6 ? "var(--away-color)" : "var(--border)",
      ...
      {error && (
        <div style={{ padding: "10px 16px", borderRadius: 6, background: "rgba(255,82,82,0.1)", border: "1px solid #ff5252", color: "#ff5252", fontSize: 13, marginBottom: 16 }}>
          {error}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="panel" style={{ padding: 32, textAlign: "center", color: "var(--ink-dim)" }}>
          ...
          {lobbies.map((lobby) => (
            ...
            >
              {lobby.hostTeam.logoSvg && (
                <div style={{ width: 40, flexShrink: 0 }} dangerouslySetInnerHTML={{ __html: lobby.hostTeam.logoSvg }} />
              )}
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "var(--display)", fontSize: "1rem", textTransform: "uppercase" }}>
                  {lobby.hostTeam.name}
                </div>
              ...
              <div style={{ fontFamily: "var(--mono)", fontSize: "1.4rem", letterSpacing: 4, color: "var(--ws-gold)" }}>
                {lobby.code}
              </div>
              <button
                onClick={() => { setJoinCode(lobby.code); joinLobby(); }}
                {...ctaHover}
                style={{
...
function Centered({ children }: { children: React.ReactNode }) {
  return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 48, color: "var(--ink)" }}>{children}</div>;
}
